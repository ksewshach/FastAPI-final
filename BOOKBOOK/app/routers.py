...
from schemas import ChapterCreateSchema, ChapterUpdateSchema
from models import ChapterModel
from sqlalchemy.orm import Session
from database import engine
from sqlalchemy import select, insert

tasks_router = APIRouter(prefix='/api/v1/chapters') # префикс для разработчика чисто, чтобы понимали что тут находится какой-то интерфейс 
...
def get_list_task(request: Request): #Этот реквест нужно обязательно прописывать, в некоторых гайдах его нет
    ...
    stmt = select(ChapterModel) # (sql запрос по форме запросить таблицу таск модел)
...
def create_task(request: Request, task: ChapterCreateSchema):
    session = Session(engine) # экземпляр класса, который позволяет подключиться к БД
    stmt = insert(ChapterModel).values(title=task.title,  # insert импортируется из sqlalchemy,
...
def update_task(request: Request, task_id: int, task_change: ChapterUpdateSchema): # task_id:int получаем айди к изменению у пользователя, 
    session = Session(engine)
    stmt = select(ChapterModel).where(ChapterModel.id==task_id) # формируем запрос: мы хотим получить из таблички таск модел запись по айдишнику
